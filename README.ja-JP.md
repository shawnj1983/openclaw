...
> モデルレジストリに関する注意: OpenClaw は Gateway 起動時に `openclaw.json` を元に `~/.openclaw/agents/<agentId>/agent/models.json` を再生成します。  
> 再起動後のモデル能力メタデータの後退を防ぐため、ClawX は起動前に既存 `models.json` の一致モデルから `input` / `reasoning` / `contextWindow` / `maxTokens` / `cost` を `openclaw.json` 側へ再反映します。
